...
import time
import argparse
from load_data import NUM_WRITERS, NUM_CHANNEL
...
def all_data_loader():
    ...
    print(sum([len(data_train.data_dict[i]) for i in data_train.data_dict.keys()]))
...
def train(loss_saver, train_loader, model, dis_opt, gen_opt, rec_opt, cla_opt, epoch):
    model.train()
    loss_dis = list()
    loss_dis_tr = list()
    loss_cla = list()
    ...
    for train_data_list in train_loader:
        '''rec update'''
        rec_opt.zero_grad()
        l_rec_tr = model(train_data_list, epoch, 'rec_update', cer_func=cer_tr)
        rec_opt.step()
        ...
        '''dis update'''
        dis_opt.zero_grad()
        l_dis_tr = model(train_data_list, epoch, 'dis_update', folder_name='gen_imgs/' + folder_name)
        dis_opt.step()
    ...
    loss_saver.save_to_csv([epoch, loss_dis_tr, loss_dis, loss_cla_tr, loss_cla, loss_rec_tr, loss_rec, loss_l1], mode="train")
...
def test(loss_saver, test_loader, epoch, modelFile_o_model):
    ...
    for test_data_list in test_loader:
        l_dis, l_cla, l_rec = model(test_data_list, epoch, 'eval', cer_func=[cer_te, cer_te2], folder_name='gen_imgs/' + folder_name)
    ...
    loss_saver.save_to_csv([epoch, loss_dis, loss_cla, loss_rec], mode="eval")
...
def main(train_loader, test_loader, num_writers):
    model = ConTranModel(num_writers, show_iter_num, OOV).to(gpu)
    loss_saver_train = LossSaver('train', BATCH_SIZE, NUM_CHANNEL)
    loss_saver_test = LossSaver('eval', BATCH_SIZE, NUM_CHANNEL)

    if CurriculumModelID > 0:
        model_file = 'saved_weights/' + folder_name + '/contran-' + str(CurriculumModelID) +'.model'
    ...
    for epoch in range(CurriculumModelID, epochs):
        print(epoch)
        cer = train(loss_saver_train, train_loader, model, dis_opt, gen_opt, rec_opt, cla_opt, epoch)

        if epoch % MODEL_SAVE_EPOCH == 0:
            folder_weights = 'saved_weights/' + folder_name 
            if not os.path.exists(folder_weights):
                os.makedirs(folder_weights)
            torch.save(model.state_dict(), folder_weights + '/contran-%d.model'%epoch)

        if epoch % EVAL_EPOCH == 0:
            test(loss_saver_test, test_loader, epoch, model)
        ...
        if EARLY_STOP_EPOCH is not None:
            ...
            if min_count >= EARLY_STOP_EPOCH:
                ...
                model_url = 'saved_weights/' + folder_name + '/contran-'+str(min_idx)+'.model'
                os.system('mv '+model_url+' '+model_url+'.bak')
                os.system('rm saved_weights/' + folder_name + '/contran-*.model')
                break

    loss_saver_test.close()

def rm_old_model(index):
    models = glob.glob('saved_weights/' + folder_name + '/*.model')
    for m in models:
        epoch = int(m.split('.')[0].split('-')[1])
        if epoch < index:
            os.system('rm saved_weights/' + folder_name + '/contran-'+str(epoch)+'.model')
