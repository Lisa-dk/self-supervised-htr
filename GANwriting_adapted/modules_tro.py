...
from load_data import OUTPUT_MAX_LEN, IMG_HEIGHT, IMG_WIDTH, vocab_size, num_tokens, tokens, tokenizer
import cv2
...
def write_image(xg, pred_label, gt_img, gt_label, tr_imgs, xg_swap, pred_label_swap, gt_label_swap, folder_name, title, num_tr=2):
    folder = folder_name
    ...
    for i in range(batch_size):
        ...
        gt_text_swap = tokenizer.decode(gt_text_swap)
        pred_text = tokenizer.decode(pred_text)
        pred_text_swap = tokenizer.decode(pred_text_swap)
...
class TextEncoder_FC(nn.Module):
    ...
    def forward(self, x, f_xs_shape):
        ...
        if padding_reps:
            padding = torch.full((1, 1), tokens['PAD_TOKEN'], dtype=torch.long)
            ...
            embedded_padding_char = Fun.one_hot(padding, num_classes = vocab_size).float().cuda()
