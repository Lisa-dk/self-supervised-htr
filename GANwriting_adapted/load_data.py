...
tokenizer = Tokenizer(chars=charset_base, max_text_length=OUTPUT_MAX_LEN)

'''The folder of IAM word images, please change to your own one before run it!!'''
img_base = './data/iam/words/'
print(os.listdir('../GANwriting/'))
text_corpus = '../GANwriting/corpora_english/brown-azAZ.tr'


with open(text_corpus, 'r') as _f:
    text_corpus = _f.read().split()

src = 'Groundtruth/gan.iam.gan_tr_va.filter27'
tar = 'Groundtruth/gan.iam.gan_test.filter27'
...
class IAM_words(D.Dataset):
    ...
    def new_ed1(self, word_ori):
        ...
        word_new = tokenizer.decode(word)
        word_new = word.replace(tokenizer.GO_TK, "")
        word_new = word.replace(tokenizer.END_TK, "")
        word_new = word.replace(tokenizer.UNK_TK, "")
        # print(word_new)
        # new_word = edits1(word)
        new_word_new = edits1(word_new)
        # label = np.array(self.label_padding(new_word, num_tokens))
        label_new = np.array(tokenizer.encode(new_word_new))
        # print(f"prev proc shape: {label.shape}, new proc shape: {label_new.shape}")
        return label_new
    ...
    def __getitem__(self, wid_idx_num):
        ...
        for word in words:
            ...
            label = tokenizer.encode(' '.join(word[1:]))
            # print(label)
            # print(f"types: {type(label)}, {type(label_new)}")
            # print(f"prev label proc shape: {label.shape}, new label proc shape: {label_new.shape}")
            wids.append(wid)
            idxs.append(idx)
            imgs.append(img)
        ...
        if self.oov:
            ...
            label_xt = np.array(tokenizer.encode(label_xt))
            label_xt_swap = np.random.choice(text_corpus)
            # label_xt_swap = np.array(self.label_padding(label_xt_swap, num_tokens))
            label_xt_swap = np.array(tokenizer.encode(label_xt_swap))
            # print(f"old swap shaped: {label_xt.shape}, {label_xt_swap.shape}")
            # print(f"new shaped: {label_xt_new.shape}, {label_xt_swap_new.shape}")
        else:
    ...
    def read_image_single(self, file_name):
        ...
        outImg = np.ones((IMG_HEIGHT, IMG_WIDTH), dtype='float32') * background
        start = int((ht / 2) - (new_size[1] / 2))
        end = start + new_size[1]
        outImg[start:end, :img_width] = img
        outImg = outImg.astype('float32')

        outImg = outImg/255. # 0-255 -> 0-1
        outImg = 1. - outImg

        m, s = 0.5, 0.5
        outImgFinal = (outImg - m) / s
