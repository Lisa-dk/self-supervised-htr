...
class ConTranModel(nn.Module):
    ...
    def forward(self, train_data_list, epoch, mode, cer_func=None, folder_name='imgs'):
        tr_domain, tr_wid, tr_idx, tr_img, tr_img_width, tr_label, img_xt, label_xt, label_xt_swap = train_data_list
        # print(vocab_size)
        label_xt_one_hot = Fun.one_hot(label_xt, num_classes = vocab_size).float()
        # print("one hot shape: ", label_xt_one_hot.shape)
        
        # print(label_xt_one_hot)
        label_xt_swap_one_hot = Fun.one_hot(label_xt_swap, num_classes = vocab_size).float()
        ...
        label_xt_one_hot = label_xt_one_hot.to(gpu)
        label_xt_swap_one_hot = label_xt_swap_one_hot.to(gpu)
        ...
        if mode == 'rec_update':
            ...
            # print("one hot shape", label_xt_one_hot.shape)
            f_xt, f_embed = self.gen.enc_text(label_xt_one_hot, f_xs.shape) # b,4096  b,512,8,27
            ...
            f_xt_swap, f_embed_swap = self.gen.enc_text(label_xt_swap_one_hot, f_xs.shape)
            ...
            if self.iter_num % self.show_iter_num == 0:
                with torch.no_grad():
                    pred_xt = self.rec(xg, label_xt, img_width=torch.from_numpy(np.array([IMG_WIDTH]*batch_size)))
                    pred_xt_swap = self.rec(xg_swap, label_xt_swap, img_width=torch.from_numpy(np.array([IMG_WIDTH]*batch_size)))
                write_image(xg, pred_xt, img_xt, label_xt, tr_img, xg_swap, pred_xt_swap, label_xt_swap, folder_name, 'epoch_'+str(epoch)+'-'+str(self.iter_num))
            return l_total
            ...
            with torch.no_grad():
                ...
                pred_xt = self.rec(xg, label_xt, img_width=torch.from_numpy(np.array([IMG_WIDTH]*batch_size)))
                pred_xt_swap = self.rec(xg_swap, label_xt_swap, img_width=torch.from_numpy(np.array([IMG_WIDTH]*batch_size)))
                write_image(xg, pred_xt, img_xt, label_xt, tr_img, xg_swap, pred_xt_swap, label_xt_swap, folder_name, 'eval_'+str(epoch)+'-'+str(self.iter_num))
                self.iter_num += 1
                '''dis loss'''
                l_dis_ori = self.dis.calc_gen_loss(xg)
                l_dis_swap = self.dis.calc_gen_loss(xg_swap)
